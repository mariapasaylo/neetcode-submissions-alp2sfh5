    ...
    def search(self, word: str) -> bool:
        ...
        #define dfs(j, root)
            ...
            #iterate through j to length of word
                ...
                #if c is "."
                    #iterate through each child value in dictionary NOTE: iterate through all possible characters
        ...
        def dfs(j, root):
            ...
            for i in range(j,len(word)):
                c = word[i]
                if c == ".":
                    for child in cur.vals.values(): #check all possible characters
                        if dfs(i+1, child): #if any path works
                            return True #then word is in dictionary
                    return False #if loop ends and no path works thenr return false
                else:
